...
import {
  PageWrapper,
  Header,HeaderLeft,TeapotIcon,HeaderTitle,HeaderSubtitle,TeaSelector,
//   TeaDropdownBtn,
  TeaIcon,TeaName,ChevronIcon,
  DropdownMenu,DropdownItem,DropdownImg,
  StepNav,StepList,StepItem,StepDot,StepDotInner,StepConnector,StepLabel,
  VideoSection,VideoWrapper,PlayBtn,VideoMeta,StepContent,StepBadge,StepTitle,StepDesc,StepIllustration,
  TipBox,TipLeaf,TipText,NavRow,PrevBtn,NextBtn,
...
const TEA_LIST = [
  { id: "nokcha", name: "녹차", img: leavesImg, color: "#4a7c59" },
  { id: "ginger", name: "생강차", img: gingerImg, color: "#b07d3a" },
  { id: "omija", name: "오미자차", img: omijaImg, color: "#c0392b" },
  { id: "gughwa", name: "국화차", img: gughwaImg, color: "#c9a227" },
  { id: "mogwa", name: "모과차", img: mogwaImg, color: "#8a9a3a" },
  { id: "dechu", name: "대추차", img: dechuImg, color: "#7d3a1a" },
  { id: "plum", name: "매실차", img: plumImg, color: "#5a7a3a" },
  { id: "yuja", name: "유자차", img: yujaImg, color: "#c4a000" },
];
...
export default function TeaGuide() {
  ...
  return (
    <PageWrapper>
      <Header>
        <HeaderLeft>
          <TeapotIcon src={kettleImg} alt="teapot" />
          <div>
        ...
        <div ref={dropdownRef} style={{ position: "relative" }}>
          <TeaSelector onClick={() => setDropdownOpen((v) => !v)}>
            <span style={{ fontSize: "11px", color: "#888", marginBottom: "2px" }}>선택한 차</span>
        ...
        <StepList>
          {STEPS.map((s, i) => (
            <div key={s.num} style={{ display: "flex", alignItems: "flex-start" }}>
              <StepItem onClick={() => handleGoToStep(i)} active={i === currentStep}>
                <StepDot active={i === currentStep} done={i < currentStep}>
                  <StepDotInner active={i === currentStep} done={i < currentStep}>
                    {i < currentStep ? "✓" : s.num}
                  </StepDotInner>
                </StepDot>
                <StepLabel active={i === currentStep}>{s.label}</StepLabel>
              </StepItem>
              {i < STEPS.length - 1 && (
        ...
        <VideoWrapper>
          {playing ? (
            ...
            <>
              ...
              />
              <PlayBtn onClick={handlePlay}>▶</PlayBtn>
      ...
      <StepContent>
        ...
        <TipBox>
          <TipLeaf>🍃</TipLeaf>
          <div>
            <div style={{ fontWeight: 700, fontSize: "13px", marginBottom: "3px", color: "#4a7c59" }}>TIP</div>
            <TipText>{step.tip}</TipText>
          </div>
          <img src={step.mascot} alt="mascot" style={{ width: "60px", height: "60px", objectFit: "contain", marginLeft: "auto" }} />
        </TipBox>
      </StepContent>
